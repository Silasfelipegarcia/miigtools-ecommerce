...
class Header extends \Opencart\System\Engine\Controller {
	...
	public function index(): string {
		$this->document->addLink('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600&family=Montserrat:wght@500;600;700;800&display=swap', 'stylesheet');
		$this->document->addStyle('catalog/view/stylesheet/miigtools.css');
		...
		$apple_touch = 'catalog/miigtools-apple-touch.png';
		$icon_192 = 'catalog/miigtools-icon-192.png';

		$data['icon_apple'] = is_file(DIR_IMAGE . $apple_touch)
			? $this->config->get('config_url') . 'image/' . $apple_touch
			: $data['icon'];

		$data['icon_192'] = is_file(DIR_IMAGE . $icon_192)
			? $this->config->get('config_url') . 'image/' . $icon_192
			: $data['icon'];
		...
		if ($this->customer->isLogged()) {
			...
			$data['wishlist_count'] = $this->model_account_wishlist->getTotalWishlist($this->customer->getId());
