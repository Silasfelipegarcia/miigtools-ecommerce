        ...
        $.ajax({
            ...
            success: function (json, textStatus) {
                ...
                if (typeof json['total'] !== 'undefined') {
                    var act = String(action || '').replace(/&amp;/g, '&');
                    if (act.indexOf('wishlist') !== -1 && $('#wishlist-total').length) {
                        var $wl = $('#wishlist-total');
                        $wl.attr('title', json['total']).attr('aria-label', json['total']);
                        var match = String(json['total']).match(/\((\d+)\)/);
                        var n = match ? parseInt(match[1], 10) : 0;
                        var $badge = $wl.find('.miig-icon-badge');
                        if (n === 0) {
                            $badge.remove();
                        } else if ($badge.length) {
                            $badge.text(n);
                        } else {
                            $wl.append('<span class="miig-icon-badge">' + n + '</span>');
                        }
                    }
                }
            },
